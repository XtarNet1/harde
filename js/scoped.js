...
(function() {
    ...
    function fetchAndApplyScopedCSS(node, attrName, prefix) {
        ...
        if (src) {
          fetch(src,{
            ...
            node.textContent = css; // Insert the CSS into the style element
            upgradeSheet(node.sheet, prefix); // Apply the scoping logic
            applyMode(node.parentNode, attrName, true); // Apply the scoped attribute or class
          }).catch(error => {
            console.error('Failed to fetch scoped CSS:', error);
          });
        }
      }
    ...
    function upgrade(node) {
      ...
      if (node.hasAttribute('src')) {
        fetchAndApplyScopedCSS(node, attrName, prefix);
      } else {
        // Inline styles or already fetched external styles
